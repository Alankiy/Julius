...
RUN apt-get update && install -y python-pyaudio python3-pyaudio
RUN curl -OL http://jaist.dl.sourceforge.jp/julius/71011/dictation-kit-$JULIUS_VERSION.zip
RUN unzip dictation-kit-$JULIUS_VERSION.zip && cd /dictation-kit-$JULIUS_VERSION && chmod 755 *.sh && chmod 755 bin/linux/* && ./bin/linux/julius -C main.jconf -C am-dnn.jconf -dnnconf julius.dnnconf -module -input adinnet &
